...
namespace GrapeCity.ActiveReports.Samples.Rtf
{
	...
	public sealed class RtfDesigner : CustomReportItemDesigner, IValidateable
	{
		...
		public bool CanGrow
		{
			get => GetCustomPropertyAsBoolean(CAN_GROW_FIELD_NAME, false);
			set => SetCustomProperty(CAN_GROW_FIELD_NAME, value.ToString());
		}

		[DefaultValue(false)]
		public bool CanShrink
		{
			get => GetCustomPropertyAsBoolean(CAN_SHRINK_FIELD_NAME, false);
			set => SetCustomProperty(CAN_SHRINK_FIELD_NAME, value.ToString());
		}
		...
		private string GetCustomPropertyAsString(string propertyName)
		{
			var customProperty = ReportItem.CustomProperties[propertyName];
		...
		private bool GetCustomPropertyAsBoolean(string propertyName, bool defaultValue)
		{
			var customProperty = ReportItem.CustomProperties[propertyName];
			if (customProperty != null)
				return Convert.ToBoolean(customProperty.Value.ToString());
			
			return defaultValue;
		}
		
		private void SetCustomProperty(string name, ExpressionInfo value)
		{
			var customProperty = ReportItem.CustomProperties[name];
			if (customProperty != null)
