	...
	public class ValidationUtils
	{
		...
		public static void ValidateLength(Length value, Length min, Length max)
		{
			...
			if (!value.IsValid)
			{
				...
				throw DiagContext.RaiseException(message, new ArgumentException(message));
			}
			if (value.ToTwips() < min.ToTwips() || value.ToTwips() > max.ToTwips())
			{
				...
				throw DiagContext.RaiseException(message, new ArgumentOutOfRangeException(null, message));
			}
		}
	}
}