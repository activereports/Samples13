...
namespace GrapeCity.ActiveReports.Calendar.Design.Tools
{
	...
	internal static class Utils
	{
		...
		public static DataRegion GetParentDataRegion(IReportComponent reportComponent)
		{
			...
			{
				DiagContext.Designer.Assert(false, "Invalid report item");
				return null;
			}
			...
			DiagContext.Designer.TraceVerbose("Failed to find a data region containing item");
			return null;
		}
		...
		public static Report GetReportFromServiceProvider(IServiceProvider serviceProvider)
		{
			...
			{
				DiagContext.Designer.TraceWarning("Unable to get a report. The ServiceProvider is null.");
				return null;
			}
			...
			if (host == null)
			{
				DiagContext.Designer.TraceWarning("Unabled to get a report. Expected to be able to get a designer host");
				return null;
			}
			PageReport reportDef = host.RootComponent as PageReport;
			if (reportDef == null)
			{
				DiagContext.Designer.TraceVerbose("Unabled to get a report. Unable to get locate a report.");
				return null;
			}
		...
		private static StringCollection GetFieldsForCustomReportItemDataSet(CustomReportItem customReportItem, IServiceProvider serviceProvider)
		{
			...
			if (report == null)
			{
				DiagContext.Designer.TraceWarning("Unabled to get fields for custom report item. Unable to get a report from the service provider");
				return fields;
			}
			DiagContext.Designer.Assert(customReportItem.CustomData != null, "CustomData can not be null.");
		...
		public static PageReport GetReportDefinition(IComponent component)
		{
			...
			{
				DiagContext.Designer.TraceWarning("Invalid or unsited component");
				return null;
			}
			return pageReport;
		}
