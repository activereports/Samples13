...
namespace GrapeCity.ActiveReports.Calendar.Design.SmartPanels.Controls
{
	...
	internal sealed class DataGridComboBoxColumn : DataGridColumnStyle
	{
		...
		public DataGridComboBoxColumn(IServiceProvider serviceProvider)
		{
			...
			DiagContext.Designer.Assert(_designerHost != null, "Expected to be able to get a hold of a designer host.");
		...
		protected override void SetColumnValueAtRow(CurrencyManager source, int rowNum, object value)
		{
			...
			{
				DiagContext.Designer.TraceVerbose("Invalid position in the currency manager, ignoring set value");
				return;
			}
			try
			{
		...
		private void Flush()
		{
			...
			try
			{
				if (iCurrentRow < 0)
				{
					DiagContext.Designer.Assert(!comboBox.Visible, "Invalid combo state");
					return;
				}
		...
		private sealed class DataGridComboBox : ComboBoxEx
		{
				...
				: base(serviceProvider)
			{
				if (column == null)
				{
					DiagContext.RaiseException("Invalid data grid column", new ArgumentNullException("column"));
				}
				this.Visible = false;
				this.column = column;
			}
			...
			private sealed class ScheduledUpdate : IDisposable
			{
				...
				public static void ScheduleUpdate(DataGridComboBox combo, String value)
				{
					if (combo == null)
					{
						DiagContext.Designer.Fail("Invalid combo");
						return;
					}
					if (combo.scheduledUpdate != null)
					{
						DiagContext.Designer.TraceVerbose("There is already scheduled update, ignoring");
