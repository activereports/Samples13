...
using GrapeCity.ActiveReports.Calendar;
...
namespace GrapeCity.ActiveReports.Calendar.Rendering
{
	...
	public  class ImageLocatorService
	{
		...
		public ImageLocatorService(IReport parentReport)
		{
			...
			{
				DiagContext.Render.Fail("Unable to get the parent report for calendar report item");
				return;
			}
		...
		protected void InitializeServices()
		{
			...
			{
				DiagContext.Designer.TraceWarning("Can get report definition from the host's root component");
				return;
			}
		...
		private static Image LoadImageSafe(Stream imageStream, string key)
		{
			...
			{
				DiagContext.Designer.TraceError("Unable to load image '{0}'.", key);
				return null;
			}
		}
		...
		private Stream GetExternalImageStream(object value)
		{
			...
			catch (Exception)
			{
				...
				DiagContext.Designer.TraceError("Failed to load image from '{0}'.", value);
				return null;
			}
		}
		...
		private Stream GetEmbeddedImageStream(object value)
		{
			...
			{
				DiagContext.Designer.TraceError("Error reading embeded image data.");
				return null;
			}
		}
		...
		private EmbeddedImage FindEmbeddedImage(String valueName)
		{
			...
			{
				DiagContext.Designer.TraceVerbose("Report.EmbededImages: there are no images, embeded in this report.");
				return null;
			}
			...
			DiagContext.Data.TraceVerbose("Report.EmbededImages: failed to find embeded image with name '{0}'.", valueName);
			return null;
		}
		/// <summary>
